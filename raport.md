...
- **Cechy własne (pokemon.csv)**: definiują statystyki bazowe oraz przypisane typy poszczególnych Pokemonów.
- **Wyniki starć (combats.csv / tests.csv)**: definiują identyfikatory historycznych potyczek oraz informację o zwycięzcy.
...
| `HP`         | Numeryczny           | Atrybut informuje o puli punktów zdrowia Pokemona. Determinuje on ilość obrażeń, które Pokemon może otrzymać przed porażką.                                             |
| `Attack`     | Numeryczny           | Parametr określający siłę uderzenia.                                                                                                                                          |
| `Defense`    | Numeryczny           | Atrybut reprezentujący obronę przed atakami fizycznymi.                                                                                                                                    |
| `Sp. Atk`    | Numeryczny           | Wartość określająca siłę ataków specjalnych , które pomijają standardową obronę fizyczną przeciwnika.                                                                                 |
| `Sp. Def`    | Numeryczny           | Statystyka oznaczająca odpowiedź na `Sp. Atk`. Powala zredukować lub uodpornić na wymianę i efekty ataków specjalnych.                                                                                   |
| `Speed`      | Numeryczny           | Istotny atrybut informujący o priorytetach uderzeń.                                                              |
| `Generation` | Numeryczny           | Parametr generacji wyznaczający edycję gry w jakiej zaimplementowany został Pokemon. Przyjmuje ramy dyskretne: zbiór liczbowy 1-6.                                                                                          |
| `Legendary`  | Nominalny / Logiczny | Flaga atrybutu informująca, czy dany Pokemon zalicza się do rzadkich gatunków legendarnych. Pokemony z wartością 'True' charakteryzują się zazwyczaj znacznie wyższymi wartościami podstawowych statystyk i stanowią najsilniejszą grupę w grze.              |
...
2. **Korelacje pomiędzy wartościami atrybutów**
   ...
   **Wnioski:**
   - W przypadku zmiennej celu wyznaczającej zwycięstwo przeważnie wszystkie analizowane, różnicowe atrybuty numeryczne wykazują dodatnią korelację. W żaden sposób nie wpływają przeciwnie (nie zmniejszają szans).
   - Różnica w szybkości (`Diff_Speed`) wyróżnia się jako atrybut o zdecydowanie najwyższej korelacji (współczynnik powyżej 0.7). Wynika to z mechaniki walki, w której szybszy Pokemon atakuje jako pierwszy, co daje mu ogromną przewagę taktyczną już na samym starcie.
   - Atrybuty ofensywne (`Diff_Attack`, `Diff_Sp. Atk`) stanowią kolejną grupę ze stosunkowo silną korelacją (oscylującą w okolicy 0.5). Silny atak zadany zaraz po zapewniającej inicjatywę różnicy szybkości przekłada się bezpośrednio na dominację w walce.
   - Atrybuty związane typowo ze statystykami wytrzymałości i obrony (`Diff_HP`, `Diff_Defense`, `Diff_Sp. Def`) wykazują zauważalnie mniejszą korelację z wygraną (np. 0.34 dla `HP`). Oznacza to, że przewaga ofensywa na ogół lepiej determinuje ostateczny wynik starcia niż wybór Pokemona skupionego wyłącznie na przyjmowaniu obrażeń i wysokim zdrowiu.
...
### Podsumowanie
Na początku przeprowadzono analizę rozkładów, z której wynika, że atrybuty numeryczne nie mają rozkładów normalnych, a klasy rozkładu w turniejach cechują się zmiennością. W związku z tym zastosowano metodę Spearmana do analizy korelacji. Z analizy korelacji wynika, że występują grupy atrybutów silnie ze sobą skorelowane – pierwsza grupa: `Attack`, `Defense`, `Sp. Atk`, `Sp. Def`, druga grupa obejmuje powiązanie `HP` z bazowymi statystykami. Ze względu na dużą korelację danych atrybutów, przekazują one podobną informację (definiując ogólną siłę Pokemona związaną m.in. z ewolucją czy przynależnością do klasy `Legendary`), co może być redundantne dla modelu.

Następnie obliczono korelację względem atrybutu celu (`First_won`). Atrybutami, które najsilniej korelują z wynikiem starcia są w kolejności: `Diff_Speed`, `Diff_Attack`, `Diff_Sp. Atk`, `Diff_Sp. Def`, `Diff_HP` i `Diff_Defense`. Są to przeważnie atrybuty należące do grup silnie skorelowanych zmiennych bojowych. W związku z czym w fazie modelowania można rozważyć usunięcie niektórych atrybutów pozostawiając tylko kluczowy w grupie, przykładowo biorąc pod uwagę wartości korelacji ze zmienną zwycięstwa – wystarczający może okazać się parametr `Diff_Speed` w asyście `Diff_Attack`.

Dodatkowo większość atrybutów (zwłaszcza różnicowych) ma korelację dodatnią z wynikiem bitwy, z czego wynika, że ich wyższe wartości częściej występują wśród przypadków ostatecznego zwycięstwa (ponieważ do obliczania korelacji wygrana reprezentowana przez pierwszego osobnika została zamieniona na 1, a dla drugiego na 0). Zgadza się to z mechaniką gry, w której przewaga szybkości i siły ataku pozwala na zadanie kluczowych obrażeń, zanim wolniejszy i słabszy przeciwnik zdąży zareagować.

Następnie dokonano oceny jakości danych. Głównym mankamentem danych jest brak dokumentacji udostępnionego "silnika" symulującego wyniki walk podanych w zbiorze – nie podano wzorów dokładnego wyznaczania obrażeń oraz ewentualnych wskaźników losowości czy mnożników żywiołów. Dlatego wyjściowo ciężko jest ze stuprocentową pewnością ocenić, czy występowanie każdego punktu w pełni ma sens, czy też nie (bez znania całego kodu symulacji). Jednakże ze względu na silną dodatnią korelację ze statusem zwycięzcy, występowanie tzw. outlierów w głównych statystykach zawsze świadczy o dużej przewadze legendarnego Pokemona, a zatem jest wysoce przydatne dla modelu. W związku z czym najlepszą decyzją docelową byłoby ścisłe pozostawienie wartości odstających w zbiorze bez ingerencji.
